...
import { webDavListing, listSpace, closeProcess, makeHydraCollection, deleteImages, ingestImages, patchImages } from "./shared";
...
const config = positionals[2] ? await Bun.file(`_sync/${positionals[2]}`).json() : undefined

if (!config) {
    throw new Error("Please submit the name of a config file in the _sync folder");
}
...
folderListing = folderListing.filter(i => i.type === "file" && mimeTypes.includes(i.mime)).map((metadata, index) => {
    const stringValues = metadata.filename.split("/").slice(1, -1).map(i => i.toLowerCase().replaceAll(/['. ]/g, "-"))
...
const imagesToBeDeleted = spaceListing.filter(image => !webdavIds.includes(image.id))
const imagesToBeIngested = folderListing.filter(file => !dlcsIds.includes(file.id))
