...
export const webDavListing = async (
  client: WebDAVClient,
  path: string,
  ...
  const timeoutHandler = () => {
    console.error("This takes very long; please check your connection.");
  };
...
export const listSpace = async (
  ...
  if (firstPage.view && full) {
    ...
    for (let page = 2; page <= numberOfPages; page++) {
      const resp = await dlcsApiCall(
        `spaces/${space}/images?page=${page}${query}`
      );
    ...
    console.log(
      `${totalItems} images found. Use --full flag to list all images.`
    );
  } else {
...
export const ingestImages = async (body: any, batches: boolean = false) => {
  ...
  if (count > batchSize) {
    ...
    console.log(
      `${count} images are added to the queue in ${chunks.length} batches...`
    );
...
export const patchImages = async (collection: any) => {
  ...
  for (const member of members) {
    ...
    const { string1, string2, string3, number1, number2, number3, origin } =
      member;
    const body = {
      string1,
      string2,
